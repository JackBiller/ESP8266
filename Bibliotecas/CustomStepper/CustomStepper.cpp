...
void CustomStepper::step(){
  ...
  if (this->stepsToGo - this->stepsGone > 0){
    ...
    this->step();
  } else if (this->mode == STEPS) this->disable();
}
...
void CustomStepper::rotate(unsigned int rotations){
  if (this->spr > 0 && this->rpm > 0){
    if (rotations > 0)  this->mode = ROTATIONS;
    else                this->mode = CONTINUOUS;
...
void CustomStepper::rotateDegrees(float degrees, boolean correction){
  if(this->spr > 0 && this->rpm > 0 && degrees > 0){
    ...
    } else this->stepsToGo = degrees/360.0*this->spr;
    this->stepsGone = 0;
    this->done = false;
    this->calcTime();
    this->step();
...
void CustomStepper::setMode(byte mode){
  this->mode = mode;
}