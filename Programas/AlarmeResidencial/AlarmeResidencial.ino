...
void loop() {
  ...
  if (acionamento == HIGH && status_alarme == 1){                  //Sem movimento, mantem rele 1 sirene desligado
  ...
  */
  ...
  if (destrava){       //codigo rejeição não liberado
...
void botao_alarme(){
  if (digitalRead(bt_aciona) == LOW){
    ...
    // Desativar o sensor de presença
    status_alarme = LOW;
    noTone(buzzer);
    posPulso = false;
  }
